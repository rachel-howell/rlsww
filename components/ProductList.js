        ...
        <div className="grid grid-cols-1 gap-y-10 gap-x-6 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 xl:gap-x-8">
            {products.edges.map((product) => (
                <div className="group" onClick={()=>goToProductPage(product.node.handle)} >
                    ...
                    <Image src={product.node.images.edges[0].node.transformedSrc} alt="rls logo" width="2250" height="3000" className="h-full w-full object-cover object-center group-hover:opacity-75"/>
                    </div>
